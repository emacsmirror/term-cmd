...
Programs running in term.el can print a special escape sequence to run commands
in Emacs -- this is how directory tracking works. However, by default the
commands are hard-coded, and you can't add new ones.

This package lets you add new commands. It uses a different escape sequence to
avoid interfering with the built-in commands, but the principle is the same.
When a program prints a command, it won't show up on the screen, but will
instead be interpreted by Emacs.
...
through nested shells, multiple SSH sessions, and tmux.
